...
const UserDashboard = () => {
  ...
  const handleApply = async (campaignId) => {
    try {
      await axios.post(`${API}/api/users/apply/${campaignId}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });

      const appliedRes = await axios.get(`${API}/api/users/applied`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    const fetchData = async () => {
      try {
        const [userRes, appliedRes, recommendedRes] = await Promise.all([
          ...
          axios.get(`${API}/api/users/applied`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get(`${API}/api/users/recommended`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
            ...
            {appliedCampaigns.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {appliedCampaigns.map(campaign => (
                  <div key={campaign._id} className="border rounded-lg p-4 bg-gray-50">
            ...
            {recommendedCampaigns.length === 0 ? (
              ...
              <ul className="space-y-4">
                {recommendedCampaigns.map(campaign => (
                  <li key={campaign._id} className="border rounded p-4 bg-gray-50">
